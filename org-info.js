...
}}}}}},mkNodeFromHref:function(q){if(q.match(this.REGEX)){var g=this.REGEX.exec(q);
this.debug+=g[1]+" + "+g[2]+"\n";var c=g[2];var o=document.getElementById(c);if(null==o){this.debug+=("heading is null. Scanning stopped.\n");
return(false);}var a=o.parentNode;var h=this.SECS.length;var f=a.className.substr(8);
o.onclick=function(){org_html_manager.fold(""+h);};o.style.cursor="pointer";if(this.MOUSE_HINT){o.onmouseover=function(){org_html_manager.highlight_headline(""+h);
};o.onmouseout=function(){org_html_manager.unhighlight_headline(""+h);};}var l="javascript:org_html_manager.navigateTo("+h+")";
if(f>this.NODE.depth){this.NODE=new OrgNode(a,o,l,f,this.NODE,c);}else{if(f==2){this.NODE=new OrgNode(a,o,l,f,this.ROOT,c);
}else{var b=this.NODE;while(b.depth>f){b=b.parent;}this.NODE=new OrgNode(a,o,l,f,b.parent,c);
}}this.SECS.push(this.NODE);var k=o.getElementsByTagName("span");if(k){for(var e=0;
e<k.length;++e){if(k[e].className=="tag"){var n=k[e].innerHTML.split("&nbsp;");for(var d=0;
d<n.length;++d){var m=this.removeTags(n[d]);if(!this.TAGS[m]){this.TAGS[m]=new Array();
this.SORTED_TAGS.push(m);}this.TAGS[m].push(h);}}else{if(k[e].className.match(this.SECNUM_REGEX)){this.SECNUM_MAP[this.trim(k[e].innerHTML)]=this.NODE;
}}}}this.NODE.hide();return(l);}return(q);},build:function(){var f=this.TITLE.innerHTML;
